        ...
        return Specification { root, query, criteriaBuilder ->
            ...
            filterDtoList.forEach { filter ->
                if (filter.name == "author") {
                    val bookAuthorJoin =
                        root.join<Book, BookAuthor>("authors", JoinType.LEFT)
                ...
                if (filter.name == "authorIds" || filter.name == "genreIds") {
                    val inClause = if (filter.name == "authorIds") {
                        val bookAuthorJoin = root.join<Book, BookAuthor>(
                            "authors",
                            JoinType.LEFT
                        )
                        criteriaBuilder.`in`(bookAuthorJoin.get<Author>("author").get<Int>("id"))
                    } else {
                        val bookGenreJoin = root.join<Book, BookGenre>(
                            "genres",
                            JoinType.LEFT
                        )
                ...
                if (filter.name == "averageRating") {
                    val ratingsJoin =
                        root.join<Book, Rating>("ratings", JoinType.LEFT)
